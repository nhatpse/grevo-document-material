...
flowchart TD
    subgraph CitDash["🏠 Citizen Dashboard /dashboard"]
        CS["Stats Section: Tổng report, Pending, Collected, Points"]
        CA["Active Request Card: Hiện report đang xử lý + Map collector"]
        CR["Recent Activity: 5 report gần nhất"]
        CRW["Rewards Section: Vouchers, Points"]
    end
    ...
    subgraph Points["⭐ Points & Rewards /points"]
        P1["Tab Point History: Lịch sử tích điểm + tổng hợp tuần"]
        P2["Tab Rewards Store: Duyệt voucher available"]
        P3["Tab My Vouchers: Voucher đã đổi"]
        P4["Tab Leaderboard: Xếp hạng citizen theo điểm tuần"]
        P5["Redeem Voucher: POST /api/rewards/redeem/:voucherId"]
    end
...
flowchart TD
    subgraph ProfileFlow["👤 Profile Management"]
        ...
        P_EDIT["Edit Profile Modal: name, phone, address"]
        P_AVATAR[Upload / Remove Avatar]
        P_LOC["Saved Locations: CRUD + address autocomplete"]
        P_COMPLETION["Profile Completion %: Hiện progress bar"]
    end

    subgraph SettingsFlow["⚙️ Settings"]
        S_PWD["Change Password: verify old → set new"]
        S_EMAIL["Change Email: send code → verify OTP"]
        S_DELETE["Delete Account: confirm password"]
    end
